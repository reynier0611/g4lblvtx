...
// AllSi_vtx_serv_2lyr_Detector::ConstructMe() method
// but the convention is as mentioned cm and deg
//____________________________________________________________________________..
//
#include "AllSi_vtx_serv_2lyr_Subsystem.h"

#include "AllSi_vtx_serv_2lyr_Detector.h"
#include "AllSi_vtx_serv_2lyr_SteppingAction.h"
...
AllSi_vtx_serv_2lyr_Subsystem::AllSi_vtx_serv_2lyr_Subsystem(const std::string &name)
...
//_______________________________________________________________________
int AllSi_vtx_serv_2lyr_Subsystem::InitRunSubsystem(PHCompositeNode *topNode)
{
  ...
  // create detector
  m_Detector = new AllSi_vtx_serv_2lyr_Detector(this, topNode, GetParams(), Name());
  m_Detector->OverlapCheck(CheckOverlap());
  // create stepping action if detector is active
  if (GetParams()->get_int_param("active"))
  {
    m_SteppingAction = new AllSi_vtx_serv_2lyr_SteppingAction(m_Detector, GetParams());
  }
  return 0;
}
//_______________________________________________________________________
int AllSi_vtx_serv_2lyr_Subsystem::process_event(PHCompositeNode *topNode)
{
...
//_______________________________________________________________________
void AllSi_vtx_serv_2lyr_Subsystem::Print(const string &what) const
{
  if (m_Detector)
  {
...
//_______________________________________________________________________
PHG4Detector *AllSi_vtx_serv_2lyr_Subsystem::GetDetector(void) const
{
  return m_Detector;
}

//_______________________________________________________________________
void AllSi_vtx_serv_2lyr_Subsystem::SetDefaultParameters()
{
  // sizes are in cm
  // angles are in deg
