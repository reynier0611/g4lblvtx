...
void matscan(){
	...
	g4Reco->ApplyCommand("/control/matScan/scan");
	cout << "All done" << endl;
}

void set_phimin(const float f){phimin = f;}
void set_phispan(const float f){phispan = f;}
void set_phibins(const int i){phibins = i;}
void set_thetamin(const float f){thetamin = f;}
void set_thetaspan(const float f){thetaspan = f;}
void set_thetabins(const int i){thetabins = i;}
