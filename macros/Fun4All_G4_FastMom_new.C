...
void Fun4All_G4_FastMom_new(
		int    nEvents        = -1,		// number of events
		int    det_ver        = 2, 		// version of detector geometry
		double gemRes         = -1, 		// GEM resolution (cm)
		bool   loadRICH       = false, 		// Load the RICH geometry
		int    magnetic_field = 4, 		// 1 = uniform 1.5T, 2 = uniform 3.0T, 3 = sPHENIX 1.4T map, 4 = Beast 3.0T map
		int    particle_gen   = 1,		// 1 = particle generator, 2 = particle gun, 3 = simple particle generator, 4 = pythia8 e+p collision
		char * genpar         = "pi-",          // particle species to simulate with the simple generators
		char * outputFile     = "out_allSi")    // output filename
{
	// ======================================================================================================
	// Input from the user
	...
	// ---------------------
	// Forward RICH geometry
	EicFRichSubsystem *RICH = new EicFRichSubsystem("RICH");
	if(loadRICH) g4Reco->registerSubsystem(RICH);
	// ======================================================================================================
	if(do_projections){
	...
	if(gemRes>0){
		...
		kalman->add_phg4hits("G4HIT_EGEM",                 		// const std::string& phg4hitsNames,
				...
				gemRes,		                    		// const float radres,
				gemRes,                             		// const float phires,
